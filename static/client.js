...
window.addEventListener('load', async function() {
    ...
    // collect or restore all nav buttons when the hamburger menu is clicked
    if (location.pathname != "/login") {
        ...
        $('.nav-expand').addEventListener('click', function() {
            if (menuOpen) {
                ...
                $('aside #menu-block-left *', true).forEach(function(el) {
                    $('.nav-left').append(el);
                })
                $('aside #menu-block-right *', true).forEach(function(el) {
                    $('.nav-right').append(el);
                })
            } else {
                menuOpen = true;
                $('.nav-left *', true).forEach(function(el) {
                    $('aside #menu-block-left').append(el);
                })
                $('.nav-right *', true).forEach(function(el) {
                    $('aside #menu-block-right').append(el);
                })
